...
public class SocksConfig {
  ...
  WebSocketHandler wshbean8Chat(
    ...
    return session -> {
      ...
      Flux<Message<ChatMessage>> incomingFlux = utils.prepareIncomingFlux(
        session,
        mapper,
        repo
      );
      ...
      Flux<WebSocketMessage> sessionOutboundFlux = utils.prepareOutBoundFluxFromRaw(
        session,
        fmcsub,
        mapper
      );
  ...
  WebSocketHandler wshbean7Chat(
    ...
    return session -> {
      ...
      Flux<WebSocketMessage> sessionOutboundFlux = utils.prepareOutBoundFlux(
        session,
        fmcpubsub,
        mapper
      );
  ...
  WebSocketHandler wshbean6Chat(
    ...
    return session -> {
      ...
      IntegrationFlow standardIntegrationFlow = IntegrationFlows
        .from(((MessageChannel) pubfmcreg))
        .bridge()
        .nullChannel();
      // StandardIntegrationFlow standardIntegrationFlow = IntegrationFlows.from(incomingFlux).channel(fmcpub).get();
      if (
        integrationFlowContext.getRegistrationById("globalChatIntegration") ==
        null
      ) {
